...
zkERC20 is a powerful extension of the standard ERC20 token, incorporating zero-knowledge proof verification for minting tokens. This template allows you to create customized tokens that leverage the privacy and security benefits of zero-knowledge proofs.
...
<Steps>
  <Step title="Create Circuit and Verifier">
    ...
    4. Note down the verifier address provided by the platform.
  </Step>
  ...
  <Step title="Create ProofTypeGroth16 Verifier">
    Create a new contract that extends the ProofTypeGroth16 adapter:

    ```solidity
    import "@strobelabs/contracts/src/core/adapters/ProofTypeGroth16.sol";

    contract MyVerifier is ProofTypeGroth16 {
        constructor(address originalVerifier) ProofTypeGroth16(originalVerifier) {}
    }
    ```
  ...
  <Step title="Create zkERC20 Token">
  ...
  <Step title="Deploy zkERC20 Token">
...
<CardGroup>
  ...
  <Card title="Frontend Integration" icon="desktop" href="/guides/frontend-integration">
    Build a user interface for interacting with your zkERC20 token.
  </Card>
  <Card title="Security Best Practices" icon="shield-halved" href="/security/zk-token-security">
    Understand the security considerations for zkERC20 tokens.
  </Card>
</CardGroup>